...
- There are three main modes: **Regular Camera mode** to take picture one by one like any camera, **Motion sensor mode** that triggers pictures according to image change and **Timelapse mode** to automatically take pictures with a delay inbetween. Shifting from one mode to the other is made by pushing the **TIMELAPSE** button. It rolls over a list of 6 exposure delays for time lapse and the two other modes. In the json file, delays are entered in ms, a value of -1 indicates a return to regular camera mode, a value of -2 indicates a shift to **Motion sensor mode**. **Keep the total length of the table at 8 entries !** 
...
- **Raw recording mode** available in **time lapse mode only**, records sensor data as a single file in the **/Raw_data** folder. It needs a separate tool for extracting an animation ([provided here](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/SD/convertraw.ps1)). The 16 first bytes are keywords, image sizes (width and height in pixels) and registers, the following are raw data for one image in reading order, easy to recover with any other tool ! This mode allows recording at a solid 4 fps for long time with good light conditions. It is disabled in **Regular camera mode** (command is just ignored).
...
- Close up images of the sensor and the MAC-GBD decapped can be found [here](https://github.com/Raphael-Boichot/Game-Boy-chips-decapping-project). The sensor presents masked pixels which always return the reference voltage given by register V plus or minus a small bias (dark current). Register O is intended to cancel this bias. They occupy the lines 123 to 128 of a 128x128 image. Correcting this bias is not necessary to get clean images as it can be corrected by digital post-processing. I assume it can be interesting for full analog recording systems. In this regard, Game Boy Camera uses both registers to target a quite narrow voltage range and has many common features with full analog recording systems as it has very limited post-processing capabilities.
- The role of each registers is quite well understood now and explained [here](https://github.com/untoxa/gb-photo). The Mitsubishi M64282FP datasheet is notorious for being crap so many informations were confirmed by reading the datasheet of the M64283FP which is basically the same sensor with more features. 
- The M64282FP is known to be used only in Game Boy Camera and the M64283FP was produced but no device using it is known at the moment. Both chips are strictly pinout compatible and very probably directly software back compatibles. They have probably been used in forgotten Japanese safety cameras from the 90's.
- The flash ADC of the MAC-GBD has a voltage full scale of 0<->3.3 Volts (found by trial and error tests with original dithering matrices). Maximum voltage reached in normal use is about 3.05 volts. It can go until 3.18 volts during camera boot (may this be intentionnal or not).
- The Game Boy Camera has a [calibration procedure to adapt for each sensor](https://github.com/Raphael-Boichot/Inject-pictures-in-your-Game-Boy-Camera-saves#part-3-calibrating-the-sensor) which effect is not at all understood at the moment. It probably deduces a bias to apply in order to have consistent images with different sensors. Other say, the results you will get with the DashBoy Camera project may vary with the sensor used as I calibrate nothing on my side.
- The Game Boy Camera has two PCB revisions for its sensors. It is unclear wether it has an effect on images other than subjective. Veteran users of the camera admit that early sensors (first PCB sensor version on Japanese cameras) has more vertical amplification artifacts but there is no clear separation between revisions. Cleaning the lens generally cancels the diffrences.
- If you use a lens mod with the DashBoy camera, how dirty will look your sensor just depends on the lens aperture you use. The wider the opening, the less artifacts on the image. These speckles due to dust or surface roughness can barely be seens with a regular Game Boy Camera due to dithering but becomes very obvious in 8 bits mode where the image is much smoother.
...
- [Andreas Hahn](https://github.com/HerrZatacke) for the fruitful technical dicussions and for the [dithering pattern generator](https://herrzatacke.github.io/dither-pattern-gen/) and the Powershell scripts.
