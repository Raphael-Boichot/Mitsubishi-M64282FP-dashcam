...
- You can directly drop a M64283FP on a Game Boy Camera sensor board as replacement chip. The image will just look softer as the table for E register (edge enhancement ratio) is not the same. Basically the 2D edge enhancement effect is cancelled with the regular camera registers and this sensor. Full support for this sensor on the DashBoy Camera is currently under developpment. A camera rom hack may restore easily the correct E register.
