...
- The M64283FP has an automatic black level calibration ("clamping circuit") supposed to be active by default by using the same P, M and X registers as a Game Boy Camera. My notes about that are in comments into the code. This leads to a perfect cancellation of offset voltage (so calibration is not needed). I guess that could be interesting if the image is interpreted by a fully analog circuit. 
