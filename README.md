...
- Adding a menuing system with a single joystick was imagined, but it requires basically starting the code from scratch. Indeed, the whole device is though as a timelapse machine, so even the regular camera mode is written like a one image timelapse ! More generally, the code structure being one single giant loop for displaying/taking pictures, add a menuing system is not easy as it.
...
- Oh, and I never managed to move the writing to SD stuff on the second core (I did try just one time to be honest), so everything is handled by the first core only and each task treated in series. Once again, it must be doable with some minimal efforts but the current design fills my need as it in terms of velocity and it probably won't change much the ergonomy. Switching between the two cores is quite tricky anyway, in my limited experience.
