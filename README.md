...
- use the TADD register of the M64283FP. You should wire the TADD pin through one of the two [free gates](/PCB/128x160_PCB%20and%20schematic/Main%20PCB/Schematic_Dashboy%20camera_NiMH_2023-05-14.pdf) of the level shifter (high level side) and wire the low level side to GPIO22. See comments in code for the next steps. I assume you are nerd enough at this point to manage next steps by yourself, the code is ready, you just have to uncomment some stuff;
...
- Close up images of the sensor and the MAC-GBD decapped can be found [here](https://github.com/Raphael-Boichot/Game-Boy-chips-decapping-project). The sensor presents masked pixels which always return the reference voltage given by register V plus or minus a small bias (offset voltage) which shifts with exposure time and can vary depending on sensors. Register O (fine voltage tuning) is intended to be tuned to cancel this offset voltage and ensure consistency between sensors. Black pixels occupy the lines 123 to 128 of a 128x128 image. The Game Boy Camera has a [calibration procedure to adapt for each sensor](https://github.com/Raphael-Boichot/Inject-pictures-in-your-Game-Boy-Camera-saves#part-3-calibrating-the-sensor). It permanently modifies some sram values but the role of these data is not well documented. It anyway deduces the register O table to apply to have consistent images with different sensors at different gains and exposure times. I used here the calibration data of one particular camera I own, but measures between different cameras calibrated show very minor differences.
...
- The M64282FP sensor is known to have been massively produced for use in the Game Boy Camera. No known old new stock or other electronic device using it have been publically documented yet. On the other hand, the M64283FP, version with more features, was available for retail but no commercial device using it is known for sure at the moment. Old new stocks sometimes appears on the internet. The M64283FP is [pinout compatible](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/Docs%20and%20research/Image%20files/Sensor%20comparison.png) with the M64282FP (as long as you stay with the 8 first registers). Register tables to drive them are nearly identical except for register E. My guess is that the M64282FP was a Nintendo exclusive, simplified and solidified unit derived from the M64283FP (despite the numbering). Indeed, the mask marking on the M64283FP is **[A64283](https://drive.google.com/file/d/1iWyl2L_uUUIKef6FCX1JYUhLUcdR75x1/view)** (Mask revision A ?) while on the M64282FP it is **[B64282](https://drive.google.com/file/d/1t0iczgT00NVYwDEGJ-6-9WYSS4gzTDif/view)** (Mask revision B ?). Doing some more science fiction, the MAC-GBD has also pins with [undocumented purpose](https://github.com/Raphael-Boichot/Game-Boy-chips-decapping-project/blob/main/README.md#game-boy-camera-mac-gbd-mapper) that could have been used to push extra registers to a M64283FP sensor via the TADD pin. So maybe the initial plan for the GB camera was to use the M64283FP sensor (or one prototype having a TADD pin and extra registers) before both companies converged toward a cheaper, more robust and simplified version of sensor and mapper. The rushed datasheet of the M64282FP is perhaps another hint: why doing another datasheet for the simplified version of a sensor ?
...
- The M64283FP has an automatic black level calibration ("clamping circuit") active by default by using the same P, M and X registers as a Game Boy Camera. My notes about that are in comments into the code. This leads to a perfect cancellation of offset voltage according to my own tests (so calibration is not needed, VOUT does not drift anymore with exposure time). The M64283FP surprisingly gives a totally disgusting image when this circuit is off, so it MUST be activated by default. On the other hand, the M64282FP is totally stable in open loop (without clamping circuit).
- Contrary to the M64283FP masked pixels signal that can be used to calibrate the sensor by exactly cancelling the sensor voltage offset with exposure (the dark pixel output is not too far from voltage value given by register V), the masked pixels of the M64282FP does always return the saturation voltage (about 1.5 volts) + Vref. It does not change with exposure at all, which means it cannot be used to make any dynamic calibration (may this be required, I think not).
...
- Overall, the M64282FP is more stable, less noisy and easier to use compared to the M64283FP. The options not present in the M64282FP (random adressing and projections) are easily doable on software side so pretty pointless (even considering limited instructions of primitive CPUs), and the automatic black level calibration can be replaced by a simple pre-calculated lookup table or just set aside. So my conclusion: the 82FP is probably a simplified 83FP with useless/sketchy features removed, not the inverse, or they both derive from a prototype which should be close to the 83FP we know.
...
- Fully supporting of the "extended" capabilities of the M64283FP sensor. Well, the "extended" capabilities are automatic dark level calibration (yet embedded) and use of the extra TADD pin, which allows pushing new registers to perform hardware cropping and manage x/y projections. It is 10x easier to do this in software with the Pi Pico.
