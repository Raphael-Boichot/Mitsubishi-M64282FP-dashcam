...
## A SD card based recorder for the Mitsubishi M64282FP artificial retina, based on Raspberry Pi Pico. 
...
- Close up images of the sensor and the MAC-GBD decapped can be found [here](https://github.com/Raphael-Boichot/Game-Boy-chips-decapping-project). The sensor presents masked pixels which always return the reference voltage given by register V plus or minus a small bias (dark current). Register O is intended to cancel this bias. They occupy the lines 123 to 128 of a 128x128 image. Correcting this bias is not mandatory to get clean images as long as digital post-processing is available. In this regard, the Game Boy Camera uses registers to target a quite narrow voltage range as it has very limited post-processing capabilities (mainly handled by the MAC-GBD mapper which returns directly 2-bits dithered images).
- The role of each registers is quite well understood now and explained [here](https://github.com/untoxa/gb-photo). The Mitsubishi M64282FP datasheet is notorious for being crap so many informations were assessed by extrapolating the datasheet of the M64283FP which is basically the same sensor with more features.
- The Game Boy camera uses only 5 bytes of registers while sensor requires 8 bytes. the 3 missing register bytes (P, M and X) are hard coded into the MAC-GBD and send to the camera anyway. Their main purpose is to inject custom convolution kernels like border extraction, not in use by the camera.
- The M64282FP sensor is known to have been massively produced for use in the Game Boy Camera. No known old new stock or other electronic device using it have been publically documented yet. On the other hand, the M64283FP was produced but no device using it is known for sure at the moment. Old new stocks sometimes appears. Both chips are strictly pinout compatible and very probably directly software back compatible. One line of Japanese mobile phone ([the THZ43 Chiaro / LaPochee by Mitsubishi](https://time-space.kddi.com/ketaizukan/1999/10.html)) has used one of these sensor but it's unclear which without any tear down (the LaPochee device beeing horribly expensive, any tear down is unlikely to happen in the future).
...
- The Game Boy Camera has a [calibration procedure to adapt for each sensor](https://github.com/Raphael-Boichot/Inject-pictures-in-your-Game-Boy-Camera-saves#part-3-calibrating-the-sensor) which effect is not at all understood at the moment (it changes some sram values but the role of these data is not documented yet). It probably deduces a bias to apply in order to have consistent images with different sensors. Other say, the results you will get with the DashBoy Camera project may vary with the sensor used as I calibrate nothing on my side.
- The Game Boy Camera has two PCB revisions for its sensors. It is unclear wether it has an effect on images other than subjective or if the board revision is associated with a sensor revision. Veteran users of the camera admit that early sensors (first PCB sensor version on Japanese cameras mainly) has more vertical amplification artifacts but there is no clear separation between revisions. Cleaning the lens generally cancels the differences.
