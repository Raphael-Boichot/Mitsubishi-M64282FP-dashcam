...
- There are two main modes: **Regular Camera mode** to take picture one by one like any camera and **Timelapse mode** to automatically take pictures with a delay inbetween. Shifting from one mode to the other is made by pushing the **TIMELAPSE** button. It rolls over a list of 7 exposure delays, the 8th being the return to regular camera mode. Delays are entered in ms, a delay of -1 indicates a return to regular camera mode.
- In **Timelapse mode**, each recording session uses a different folder to store images. Timelapse mode has virtually no ending, it can go as long as the SD card is not full.
...
- [Game Boy Camera Club](https://disboard.org/fr/server/568464159050694666) on Discord for the hype and help on all new projects. You can discuss with tyhe authors here !
