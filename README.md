...
- Close up images of the sensor and the MAC-GBD decapped can be found [here](https://github.com/Raphael-Boichot/Game-Boy-chips-decapping-project). The sensor presents masked pixels which always return the reference voltage given by register V plus or minus a small bias (dark current). Register O is intended to cancel this bias. They occupy the lines 123 to 128 of a 128x128 image. Correcting this bias is not necessary to get clean images as it can be corrected by digital post-processing. I assume it can be interesting for full analog recording systems.
- The role of each registers is quite well understood now [here](https://github.com/untoxa/gb-photo). The Mitsubishi M64282FP datasheet is notorious for being crap so many informations were confirmed by reading the datasheet of the M64283FP which is basically the same sensor with more features. 
- The M64282FP is known to be used only in Game Boy Camera and the M64283FP was produced but no device using it is known at the moment. Both chips are strictly pinout compatible and very probably directly software back compatibles. They have probably been used in very old Japanese safety cameras despite their low resolution.
