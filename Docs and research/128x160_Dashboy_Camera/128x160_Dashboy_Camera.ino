...
unsigned unsigned short int HDRData[128 * 128];      // cumulative data for HDR imaging -1EV, +1EV + 2xOEV, 8 images in total
...
void setup() {
  ...
  for (int i = 1; i < 15; i++) {
    ...
    push_exposure(1);                //update exposure registers C2-C3
  }
...
#ifdef USE_SD
      store_next_ID("/Dashcam_storage.bin");  //store last known file/directory# to SD card
#endif
    }
    short_fancy_delay();
  }
...
void recording_loop() {
  ...
  if (HDR_mode == 1) {  //default is 8 pictures, beware of modifying the code in case of change
    ...
    current_exposure = get_exposure();    //store the current exposure register for later
    for (int i = 0; i < num_HDR_images; i++) {
      push_exposure(exposure_list[i]);  //vary the exposure
      take_a_picture();
    ...
    push_exposure(1);  //rewrite the old register stored before
  }
  ...
  if (TIMELAPSE_mode == 1) {
    ...
    if (files_on_folder == max_files_per_folder) {  //because up to 1000 files per folder stalling or errors in writing can happens
      files_on_folder = 0;
      store_next_ID("/Dashcam_storage.bin");  //in case of crash...
      if (RAW_recording_mode == 0) {
        Next_dir++;
      }
    }
...
void store_next_ID(const char* path) {
#ifdef USE_SD
  uint8_t buf[4];
  File Datafile = SD.open(path, FILE_WRITE);
  Datafile.seek(0);
...
#ifdef USE_TFT
  current_exposure = get_exposure();  //get the current exposure register for TFT display
...
void init_sequence() {  //not 100% sure why, but screen must be initialized before the SD...
...
#ifdef USE_SD
  if (SD.begin(CHIPSELECT)) {  // the initialisation of SD without SD card inserted slows down the code, it is not a normal state for the device
    SDcard_READY = 1;
  } else {
    SDcard_READY = 0;
  }
  ...
  if ((SDcard_READY == 0) | (sensor_READY == 0)) {  //get stuck here if any problem to avoid further board damage
    ...
    img.pushSprite(x_ori, y_ori);  // dump image to display, sluggish when SD card error is detected, I cannot do much for it

    while (1) {  // infinite loop to avoid any further hardware damage in case of connecting issue
